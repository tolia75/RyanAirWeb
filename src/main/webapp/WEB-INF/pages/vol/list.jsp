...
<body class="container">
	...
	<table class="table">
		...
		<c:forEach var="v" items="${vols1}">
			<tr>
				<td>${v.id}</td>
				<td><fmt:formatDate value="${v.dateDepart}"
						pattern="dd/MM/yyyy" /></td>
				<td><fmt:formatDate value="${v.heureDepart}" pattern="hh:mm" /></td>
				<td>${v.depart.nom}</td>
				<td><a class="btn btn-warning" href="editdepart?id=${v.id}"
					id="btn1">modifier départ</a></td>
				<c:choose>
					<c:when test="${v.escales.isEmpty()}">
						<td></td>
						<td></td>
						<td></td>
						<td><a class="btn btn-warning" href="editscale"
								id="btn2">ajouter escale</a></td>
					</c:when>
					<c:otherwise>
						<c:forEach var="e" items="${v.escales}">
							<td><fmt:formatDate value="${e.heureArrivee}"
									pattern="hh:mm:ss" /></td>
							<td><fmt:formatDate value="${e.heureDepart}"
									pattern="hh:mm:ss" /></td>
							<td>${e.key.aeroport.nom}</td>
							<td><a class="btn btn-warning" href="editscale?id=${e.key}"
								id="btn2">modifier escale</a></td>
						</c:forEach>
					</c:otherwise>
				</c:choose>
				<td><fmt:formatDate value="${v.dateArrivee}"
						pattern="dd/MM/yyyy" /></td>
				<td><fmt:formatDate value="${v.heureArrivee}" pattern="hh:mm" /></td>
				<td>${v.arrivee.nom}</td>
				<td><a class="btn btn-warning" href="editarrivee?id=${v.id}"
					id="btn3">modifier arrivée</a></td>
				<!-- On explique vouloir editer et sur qui  -->
				<td></td>
				<td></td>
				<td></td>
				<td></td>
